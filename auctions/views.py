...
def myposts(request):
    posts = Post.objects.filter(user=request.user).order_by('-time_create')
    return render(request,"auctions/myposts.html",{
        "posts": posts,
    })
...
def save(request):
    if request.method == "POST":
        ...
        mileages = Mileage.objects.get(mileage=mileage)
